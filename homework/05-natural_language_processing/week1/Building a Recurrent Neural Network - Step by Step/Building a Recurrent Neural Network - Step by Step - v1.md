...
RNN 在自然语言处理方面十分有效，因为 记忆细胞的作用，非双向神经网络，可以学习前面部分向后传递，双向神经网络 BRNN 可以学习前面的以及后面的序列信息。

**Notation**:
- Superscript $[l]$ denotes an object associated with the $l^{th}$ layer. 层
    - Example: $a^{[4]}$ is the $4^{th}$ layer activation. 第 4 层的激活值 $W^{[5]}$ and $b^{[5]}$ are the $5^{th}$ layer parameters. 第 5 层的参数

- Superscript $(i)$ denotes an object associated with the $i^{th}$ example. 样本 
    - Example: $x^{(i)}$ is the $i^{th}$ training example input. 第 i 个输入 训练样本

- Superscript $\langle t \rangle$ denotes an object at the $t^{th}$ time-step. 时间步
    - Example: $x^{\langle t \rangle}$ is the input x at the $t^{th}$ time-step. $x^{(i)\langle t \rangle}$ is the input at the $t^{th}$ time-step of example $i$.
    
- Lowerscript $i$ denotes the $i^{th}$ entry of a vector. 一个向量的 第 i 个值
...
Here, $W_f$ are weights that govern the forget gate's behavior. We concatenate 连接 $[a^{\langle t-1 \rangle}, x^{\langle t \rangle}]$ and multiply by $W_f$. The equation above results in a vector $\Gamma_f^{\langle t \rangle}$ with values between 0 and 1. This forget gate vector will be multiplied element-wise by the previous cell state $c^{\langle t-1 \rangle}$. So if one of the values of $\Gamma_f^{\langle t \rangle}$ is 0 (or close to 0) then it means that the LSTM should remove that piece of information (e.g. the singular subject) in the corresponding component of $c^{\langle t-1 \rangle}$. If one of the values is 1, then it will keep the information. 若遗忘门的值是 0 或接近 0 那么 移除掉  $c^{\langle t-1 \rangle}$ 的信息
